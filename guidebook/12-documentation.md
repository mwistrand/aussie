...
The Aussie API Gateway maintains two entirely separate documentation trees, along with extensive Javadoc that embeds architectural reasoning directly in the source. This is not accidental. Each of these practices solves a specific class of problem that a single README cannot.
...
The first sentence of each document declares its audience. This is deliberate, as someone landing on the wrong document can immediately redirect themselves.
...
### What a Junior or Mid-Level Engineer Might Do Instead

Write a single "Rate Limiting" page that mixes algorithm selection advice with client retry code. The result is a document that overwhelms consumers with infrastructure details they cannot act on, and buries the operational knobs that platform engineers need in the middle of client-side code samples. Neither audience reads the whole thing. Both miss the parts relevant to them. Audience segmentation is not a staff insight; it is a senior skill. The staff contribution is the discipline to maintain parallel documentation trees and the structural approach to routing information to the right consumer.

### Trade-offs

Maintaining two documentation trees for the same feature doubles the documentation surface area. When the rate limit response format changes, you must update both `docs/platform/rate-limiting.md` and `docs/api/rate-limiting.md`. This is real overhead. However, when the alternative is a single, confused document neither audience trusts, the trade-off is worth it.
...
Add a "Troubleshooting" header at the bottom of the document with a single bullet: "Check the logs." During an incident at 3 AM, this is not helpful. Seniors who have been on-call know this. They have been the person at 3 AM who found a useless troubleshooting section. The symptom-first structure, the ordered diagnostic paths, the exact log category strings all reflect operational experience, not just documentation effort. A junior engineer documents what the system does when it works. A senior documents what to do when it does not.
...
Document the happy path ("bloom filter rebuilds every hour") and omit failure scenarios. The implicit assumption is that operators will reason about failure modes from the implementation. They will not, because they are reading documentation precisely because they do not have time to read the implementation. A senior who has operated distributed systems documents failure modes because they have lived them. The explicit failure scenario table (with the bold "security risk" annotation for the empty-filter case) reflects operational judgment that a junior engineer has not yet developed.
...
The phrase "before authentication" is a design decision. Rate limiting could run after authentication, which would allow per-user limits but would also mean that every abusive request incurs the full cost of token validation. The Javadoc explains the trade-off: we reject excessive traffic _before_ incurring authentication overhead.
...
Point operators to `aussie --help` and trust that the CLI's built-in documentation is sufficient. Built-in help is sufficient for discovery. It is insufficient for incident response, where you need curated sequences of commands, not alphabetical lists of flags. A senior who has handled a credential compromise incident knows exactly why the ordered command sequences matter. The staff contribution is treating the incident response timeline as a documentation requirement; not just what commands exist, but which commands to run in which order when something is on fire.
...
Add a line to the README: "Make sure to use strong secrets in production." This is technically correct but practically useless. It does not tell you which values are secrets, what format they should be in, how to generate them, or how often to rotate them. A senior who has managed production secrets knows that the rotation cadence, format requirements, and generation instructions are the hard part, and that they will be asked these questions in a Slack thread at the worst possible moment. The secrets inventory table is a senior contribution. The defense-in-depth documentation that explains the startup guard as a safeguard rather than a bug is the staff contribution: it connects the code's behavior to the deployment model, so operators understand what the system is doing on their behalf.
