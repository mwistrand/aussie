...
The filter reads a global `GatewayCorsConfig` from CDI as the gateway policy and service fallback. If CORS is disabled or the config is not resolvable, it calls `rc.next()` and passes through. If the request carries no `Origin` header, it is not a CORS request and passes through. Otherwise, a matched route uses the cached snapshot's service registration `cors` policy, falling back to the gateway policy when the service has none. Platform-owned and unknown paths use only the gateway policy; a service registration cannot grant cross-origin access to them. The filter validates the origin and request method, then adds the appropriate response headers (`Access-Control-Allow-Origin`, `Access-Control-Allow-Methods`, `Access-Control-Allow-Headers`, `Access-Control-Allow-Credentials`, `Access-Control-Max-Age`). When credentials are allowed, the filter echoes the specific origin rather than returning `*`, and sets `Vary: Origin` to prevent cache poisoning.
...
This deserves clarification, because it is the kind of ordering subtlety that causes real bugs. In the Vert.x `@RouteFilter` convention, higher numbers run first. So the actual Vert.x execution order is:
...
On rate limit exceeded, the filter records metrics, dispatches a `SecurityEvent.RateLimitExceeded` event, and sets OpenTelemetry span attributes. Notably, it sets `StatusCode.OK` on the span rather than `StatusCode.ERROR`, because rate limiting is expected behavior, not a failure condition. Setting `ERROR` would trigger error-rate alerts and pollute the error budget.
