...
**Used for**: Session management. If the gateway cannot read or write a session to Redis, the operation has failed. There is no meaningful fallback, since you cannot fabricate session state.
...
Cache writes that fail are not catastrophic, since the next cache miss will repopulate from storage. Revocation writes that fail are more concerning, but the bloom filter and pub/sub mechanisms provide secondary consistency.
