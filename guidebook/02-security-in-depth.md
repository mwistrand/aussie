...
Aussie has a `dangerous-noop` authentication mode that disables all authentication. It exists for local development and integration testing, especially when platform teams in newer organizations are testing out Aussie before an authentication service exists. But if it reaches production, every endpoint in the gateway is unprotected.
...
Rate limiting requires identifying clients. The most precise identifier is the bearer token or API key. But storing raw tokens in rate limit state creates a credential exfiltration vector. Anyone with read access to the rate limiter's storage can harvest live tokens.
...
Access control runs after rate limiting. This means a rate-limited client still gets the rate limit response (429) rather than an access denied response (403). The alternative (checking access first) would leak information about which paths exist to clients that should receive 404.
