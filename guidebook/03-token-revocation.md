...
If the actual number of revoked entries exceeds the configured `expectedInsertions`, the filter is upsized to fit. This prevents the false positive rate from exceeding the configured threshold when the configuration is stale or undersized. The downside is memory: if you have 500,000 active revocations but configured 100,000, the rebuild will allocate a filter sized for 500,000, which is roughly 6 MB instead of 1.2 MB.
...
For the default 100,000 insertions at 0.1% FP rate, this means ~2.4 MB temporarily. For the large configuration (10,000,000 insertions), this means ~240 MB temporarily. This is enough to cause OOM on an undersized container.
