...
public class AccessControlEvaluator {
    ...
    @Inject
    public AccessControlEvaluator(GatewayConfig config) {
        this.config = config;
    }
    ...
    private boolean isSourceInGlobalAllowedList(SourceIdentifier source) {
        ...
        // Check IPs
        if (accessControl.allowedIps().isPresent()) {
            if (matchesIp(source.ipAddress(), accessControl.allowedIps().get())) {
                return true;
            }
        }
        ...
        // Check domains
        if (source.host().isPresent()) {
            if (accessControl.allowedDomains().isPresent()) {
                if (matchesDomain(source.host().get(), accessControl.allowedDomains().get())) {
                    return true;
                }
            }

            // Check subdomains
            if (accessControl.allowedSubdomains().isPresent()) {
                if (matchesSubdomain(source.host().get(), accessControl.allowedSubdomains().get())) {
                    return true;
                }
            }
        }
