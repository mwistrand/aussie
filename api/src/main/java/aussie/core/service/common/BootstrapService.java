...
public class BootstrapService implements BootstrapManagement {
    ...
    private boolean isAdminKey(ApiKey key) {
        return key.permissions().contains(Permission.ALL.value())
                || key.permissions().contains(Permission.SERVICE_CONFIG_CREATE.value())
                || key.permissions().contains(Permission.SERVICE_CONFIG_UPDATE.value())
                || key.permissions().contains(Permission.SERVICE_CONFIG_DELETE.value());
    }
