...
class AccessControlIntegrationTest {
    ...
    class PrivateEndpointGlobalAccessTests {
        ...
        void shouldAllowPrivateEndpointFromAllowedDomain() {
            ...
            given()
                .header("X-Forwarded-Host", "internal.test.com")
                .header("X-Forwarded-For", "127.0.0.1")
                .when()
                .get("/gateway/api/private")
                .then()
        ...
        void shouldAllowPrivateEndpointFromAllowedSubdomain() {
            ...
            // Test config allows *.internal.test.com
            given()
                .header("X-Forwarded-Host", "api.internal.test.com")
                .header("X-Forwarded-For", "127.0.0.1")
                .when()
                .get("/gateway/api/private")
                .then()
    ...
    class MixedVisibilityTests {
        ...
        void shouldHandleMixedVisibilityEndpoints() {
            var publicEndpoint = new EndpointConfig("/api/public", Set.of("GET"), EndpointVisibility.PUBLIC, Optional.empty());
            var privateEndpoint = new EndpointConfig("/api/private", Set.of("GET"), EndpointVisibility.PRIVATE, Optional.empty());
